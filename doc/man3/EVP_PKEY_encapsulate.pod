...
I<unwrappedlen> is not NULL, then the value it points to must hold the size of
the  I<unrwapped> buffer so that its size can be validated by the call, ensuring
